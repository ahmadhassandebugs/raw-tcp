...
int receive_from(int sock, char* buffer, size_t buffer_length, struct sockaddr_in *dst)
{
	...
	do
	{
		...
		if (received < 0) {
			printf("1: got packet on port: %u\n", (unsigned int)dst_port);
			break;
		}
		memcpy(&dst_port, buffer + 22, sizeof(dst_port));
		printf("2: got packet on port: %u\n", (unsigned int)dst_port);
	}
	while (dst_port != dst->sin_port);
	printf("3: received bytes: %u\n", received);
	printf("4: destination port: %d : %u\n", ntohs(dst->sin_port), (unsigned int)dst_port);
	return received;
}
